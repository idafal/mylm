...
subtitle: "Group 10: Maia Tømmerbakk, Astrid Langsrud, Ida Marie Falnes" 
...
In this compulsary exercise we are going to make an R package, called "mylm", with some of the same functionality as found in the already existing R package `lm`. We are going to test our implementations on a data set in the library `car`. 

# Part 1

## a)

The aim of part 1 is to get an overview of the dataset before making the linear regressions. The following diagram shows some diagnostic plots based on the data set from Canada in the car-library, containing information about `wages`, `education`, `age`, `sex` and `language`. 
...
The standard deviation for the estimated coefficients are set to the square root of the corresponding diagonal element of the covariance matrix. 
...
These values are presented in a "summary-table" in a similar way as the summary corresponding to `lm`. The $R^2$-value is also included in the table. The calculation of this value will be considered later. The following shows the summary of the regression using `mylm` and `lm` respectively. 
...
Also here the two regressions produce similar values. The small size of the p-values indicate that the regression is significant. The estimate of the regression coefficient for education can be understoos as the value by which the wages will increase when the value of education is increased by one. The intecept can be interpreted as the value of wages when the number of years of education equals zero.
